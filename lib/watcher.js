...
import {applyTestFileFilter, classify, getWatcherIgnorePatterns} from './globs.js';

const debug = createDebug('ava:watcher');

export async function start({api, filter, globs, projectDir, providers, reporter, stdin}) {
	for await (const {files, firstRun, previousFailures, runOnlyExclusive, updateSnapshots} of plan({api, globs, projectDir, providers, stdin})) {
...
async function * plan({api, globs, projectDir, providers, stdin}) {
	const ignorePatterns = getWatcherIgnorePatterns(globs);
	...
	let signalChanged;
	let changed = Promise.resolve(true);
	let firstRun = true;
	let runAll = true;
	...
	const debounce = setTimeout(() => {
		...
		const paths = [...dirtyPaths].filter(path => {
	...
	fs.watch(projectDir, {recursive: true}, (_, filename) => {
		dirtyPaths.add(nodePath.join(projectDir, filename));
		debug('Detected change in %s', filename);
		debounce.refresh();
	});

	while (await changed) { // eslint-disable-line no-await-in-loop, no-unmodified-loop-condition
		const doUpdateSnapshots = updateSnapshots;
		...
		yield {
			...
			updateSnapshots: doUpdateSnapshots,
		};
	}
}
